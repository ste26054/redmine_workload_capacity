	...
	def self.check_current_week#(current_date)
		...
		projects_with_wl_members.each do |project|
			project.wl_members.each do |wl_member| 
				...
				acceptable_threshold = project.wl_project_window.acceptable_check_limit.to_f/100
				danger_threshold = project.wl_project_window.danger_check_limit.to_f/100
				self.check_ratio_for_mail(project, wl_member.user, ratio, acceptable_threshold, danger_threshold)
			end
		end	
	end


	def self.check_ratio_for_mail(project, wl_user, ratio, acceptable_threshold, danger_threshold)
		# danger_threshold has to be bigger than acceptable_threshold
		unless ratio.nil?
			red_limit_minus = 1.0-danger_threshold
			red_limit_plus = 1.0+danger_threshold

			green_limit_minus = 1.0-acceptable_threshold
			green_limit_plus = 1.0+acceptable_threshold
			ratio = ratio.to_f 
