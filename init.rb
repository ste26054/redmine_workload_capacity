...
Redmine::Plugin.register :redmine_workload_capacity do
  ...
  menu :project_menu, :workload, { :controller => 'wl_boards', :action => 'index'},
                              ...
                              :after => :gantt,
                              :param => :project_id,
                              :if => Proc.new {|p| User.current.wl_manage_right?(p) || User.current.allowed_to?(:view_global_allocation_check, p)  || (Member.find_by(user_id: User.current.id, project_id: p.id).nil? ? false : Member.find_by(user_id: User.current.id, project_id: p.id).wl_member?)  }
